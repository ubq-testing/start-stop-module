...
export class Settlement extends Super {
  ...
  public async addDebit({ userId, amount, networkId, address }: AddDebit) {
    ...
    const { data: debitInsertData, error: debitError } = await this.supabase.from("debits").insert(debitData).select("*").maybeSingle();
    ...
    const { data: settlementInsertData, error: settlementError } = await this.supabase.from("settlements").insert(settlementData).maybeSingle();
  ...
  public async addCredit({
    ...
    const { data: creditInsertData, error: creditError } = await this.supabase.from("credits").insert(creditData).select("*").maybeSingle();
    ...
    if (permitInsertData) {
      const { error: creditUpdateError } = await this.supabase.from("credits").update({ permit_id: permitInsertData.id }).eq("id", creditInsertData.id);
